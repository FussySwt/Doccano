...
export class MemberApplicationService {
  ...
  public async create(projectId: string, item: MemberDTO): Promise<void> {
    try {
      const member = new MemberItem(0, item.user, item.role, item.username, item.rolename, item.is_superuser)
  ...
  public async update(projectId: string, item: MemberDTO): Promise<void> {
    try {
      const member = new MemberItem(item.id, item.user, item.role, item.username, item.rolename, item.is_superuser)
