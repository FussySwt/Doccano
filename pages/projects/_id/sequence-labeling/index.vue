  ...
  <layout-text v-if="doc.id">
    <template v-slot:header>
      <toolbar-laptop
        ...
        :is-access="isAccess"
        :approve="doc.corpus_example_content.fields.status"
        :show-approve-button="project.permitApprove"
        :total="docs.count"
        class="d-none d-sm-block"
        @click:clear-label="clear"
        @click:undo-label="clickPass(doc.id,1)"
        @click:pass="clickPass(doc.id,3)"
      ...
      >
        <div style="display:flex;">
          ...
          style="height:90%;"
            ...
            filled
          /> -->
          <v-btn v-if="getCurrentUserRole.is_annotator || getCurrentUserRole.is_project_admin" style="height:90%;" :disabled="doc.corpus_example_content.fields.status == 1 ? false : true" @click="clickPass(doc.id, 2)">提交</v-btn>
        </div>
      </toolbar-laptop>
      <toolbar-mobile
        :total="docs.count"
...
export default {
  ...
  async fetch() {
    ...
    if(doc.corpus_example_content.fields.status === 3){
  ...
  methods: {
    ...
    async add(startOffset, endOffset, labelId) {
      ...
      await this.list(this.doc.id)
      if(!this.doc.corpus_example_content.fields.status){
        this.doc.corpus_example_content.fields.status = 1
        await this.$api.changeRes(`/v1/projects/${this.$route.params.id}/examples/${this.doc.id}/states`,{status: 1})
    ...
    async clear() {
      try{
        ...
        await this.list(this.doc.id)
        if(this.doc.corpus_example_content.fields.status === 1){
          this.doc.corpus_example_content.fields.status = 0
          await this.$api.changeRes(`/v1/projects/${this.$route.params.id}/examples/${this.doc.id}/states`,{status: 0})
          // this.isSubmit = !this.isSubmit
        }
      }catch(e){
        console.log(e)
    ...
    async clickPass(id,state){
      ...
      this.doc.corpus_example_content.fields.status = state
      console.log(state)
      if(state === 3){
        this.isItemShow = false
      }
