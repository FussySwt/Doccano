...
export class APIExampleRepository implements ExampleRepository {
  ...
  async list(projectId: string, { limit = '10', offset = '0', q = '', isChecked = '',status = '' }: SearchOption): Promise<ExampleItemList> {
    const url = `/projects/${projectId}/examples?limit=${limit}&offset=${offset}&q=${q}&confirmed=${isChecked}&status=${status}`
